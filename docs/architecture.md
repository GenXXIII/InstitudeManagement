...
EF Core migrations under `Persistence/Migrations` own fresh database creation and future schema evolution. `DatabaseSchemaUpdater` remains a compatibility bridge for databases created by earlier project versions; startup upgrades that schema, inserts the initial migration baseline into `__EFMigrationsHistory`, and then applies later pending migrations without deleting existing data. New schema design belongs in `Persistence/Configurations`; the updater contains only idempotent compatibility steps. The timetable compatibility step converts only the exact hourly patterns produced by the former demo seed; other custom records remain untouched and are shown as needing rescheduling.
...
Timetable period rules are backend-owned domain policy. Monday through Friday expose morning, afternoon, and evening periods; Saturday and Sunday expose morning and afternoon periods only. Every entry owns a Year 1–4 cohort. Management presents the timetable like the other management resources: searchable and filterable data rows containing day/time, course, department/cohort, teacher, room, student count, status, and edit actions. Operations retains the room-oriented weekly visualization. Arbitrary times and Year values outside 1–4 are rejected by the backend.
...
Completed timetable delivery has its own persistent `ClassSessionRecord`. A startup check and minute-level recorder create one unique row per schedule/date after the configured end time, capturing course, teacher, classroom, cohort, academic period, totals, and a JSON snapshot of every matching student's daily attendance state. Record presents one expandable visual card per completed session with the complete Present, Late, Absent, or Permission roster. Student readers expose only completed-class attendance; Teacher and Course readers expose only completed teaching sessions. Classroom remains a relationship shown within those cards, not a separate Record navigation module.
...
The top bar owns the searchable Department scope, the All/Year 1–4 scope, and a feature-aware type-ahead search. Both scopes are preserved through sidebar navigation and applied wherever a resource has a year relationship. Data-heavy relationship inputs use searchable pickers, while small fixed enumerations remain native selects. The pale ice-blue and white visual system is shared through `app/globals.css`.
