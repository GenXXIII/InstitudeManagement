...
public sealed class TeacherOperationalRecordReader(InstituteDbContext db) : IOperationalRecordReader
{
    ...
    public async Task<IReadOnlyList<OperationalRecordDto>> GetAsync(Guid? departmentId, CancellationToken cancellationToken)
    {
        ...
        return teachers.Select(teacher =>
        {
            ...
            {
                var codes = codeFormat.Chain(teacher.TeacherCode, assignment.EnrollmentCode, assignment.OperationCode, assignment.RecordCode, assignment.HistoryCode, "teacher");
            ...
            var recordSource = assignments.Where(x => x.TeacherId == teacher.Id)
                ...
                .Select(x => string.IsNullOrWhiteSpace(x.RecordCode) ? codeFormat.Chain(teacher.TeacherCode, x.EnrollmentCode, "teacher").Record : x.RecordCode)
            ...
            return new OperationalRecordDto(teacher.Id, "Teacher", teacher.FullName, teacher.TeacherCode, attendanceStatus,
                ...
                events.Select(x => x.Item2).ToList(), Code: recordSource, PhotoDataUrl: teacher.PhotoDataUrl,
