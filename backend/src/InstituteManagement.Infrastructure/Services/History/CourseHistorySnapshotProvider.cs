...
public sealed class CourseHistorySnapshotProvider(InstituteDbContext db) : IHistorySnapshotProvider
{
    ...
    public async Task<IReadOnlyList<RecordDto>> GetAsync(CancellationToken cancellationToken) =>
        (await db.Courses.AsNoTracking().Include(x => x.Department).Include(x => x.Teacher).ToListAsync(cancellationToken)).Select(x => Create(x.Id, x.UpdatedAtUtc, Type, x.Name, x.IsActive ? "Active" : "Inactive", new { x.Code, x.Name, x.DepartmentId, department = x.Department?.Name, x.TeacherId, teacher = x.Teacher?.FullName, x.Credits, x.Capacity, status = x.IsActive ? "Active" : "Inactive", x.CreatedAtUtc, x.UpdatedAtUtc })).ToList();
}
