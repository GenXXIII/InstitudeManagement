...
public sealed class AttendanceCatalogService(InstituteDbContext db, InstituteCache cache) : CatalogFeatureBase<AttendanceResponseDto>(db, cache), IAttendanceCatalogService
{
    ...
    public override async Task<AttendanceResponseDto> UpdateAsync(Guid id, Dictionary<string, string> values, CancellationToken ct)
    {
    ...
    protected override AttendanceResponseDto Response(Guid id, IReadOnlyDictionary<string, string> values) =>
