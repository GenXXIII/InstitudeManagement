...
public sealed class ManagementService(IEnumerable<IManagementFeature> features) : IManagementService
{
    ...
    public Task<IReadOnlyList<IManagementItemDto>> GetAsync(string resource, string? search, Guid? departmentId, CancellationToken ct) => Resolve(resource).GetAsync(search?.Trim(), departmentId, ct);
    public Task<IManagementItemDto> CreateAsync(string resource, Dictionary<string, string> values, CancellationToken ct) => Resolve(resource).CreateAsync(values, ct);
    public Task<IManagementItemDto> UpdateAsync(string resource, Guid id, Dictionary<string, string> values, CancellationToken ct) => Resolve(resource).UpdateAsync(id, values, ct);
