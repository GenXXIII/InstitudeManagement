...
public sealed class CourseManagementFeature(InstituteDbContext db, InstituteCache cache) : ManagementFeatureBase(db, cache)
{
    ...
    public override async Task<IReadOnlyList<IManagementItemDto>> GetAsync(string? search, Guid? departmentId, CancellationToken ct)
    {
        ...
        return courses.Where(course => Matches(search, course.CourseCode, course.Name)).Select(course => (IManagementItemDto)new CourseResponseDto(course.Id, new CourseValuesDto(course.CourseCode, course.Name, "", "", "", "", "", "Active", course.CreateAt.ToString("yyyy-MM-dd")))).ToList();
    }
    public override async Task<IManagementItemDto> CreateAsync(Dictionary<string, string> values, CancellationToken ct)
    {
        var code = await ConfiguredRecordCodeAsync(values, "courseCode", "courses", "CRS", Db.Courses.AsNoTracking().Select(course => course.CourseCode), ct);
        values["courseCode"] = code;
    ...
    {
        var entity = await RequiredEntityAsync(Db.Courses, id, ct); var code = Required(values, "courseCode"); await EnsureUniqueAsync(Db.Courses.Where(course => course.Id != id && course.CourseCode == code), "CourseCode", ct);
    ...
    protected override IManagementItemDto Response(Guid id, IReadOnlyDictionary<string, string> values) => new CourseResponseDto(id, new CourseValuesDto(Get(values, "courseCode"), Get(values, "name"), "", "", "", "", "", Get(values, "status", "Active"), Get(values, "createAt", DateTime.UtcNow.ToString("yyyy-MM-dd"))));
}
