...
public sealed class CourseOperationReader(InstituteDbContext db, OperationContextService contextService) : IOperationModuleReader
{
    ...
    public async Task<OperationDto> GetAsync(Guid? departmentId, CancellationToken cancellationToken)
    {
        ...
        var courses = await query.OrderBy(x => x.CourseCode).Take(16).ToListAsync(cancellationToken);
        ...
        var rows = courses.Select(x => new CourseOperationDto(x.Id, x.Name, x.CourseCode, x.Teacher?.FullName ?? "—", x.Department?.Name ?? "—", x.Capacity, "Active")).ToList();
