...
public sealed class ClassroomOperationReader(InstituteDbContext db, OperationContextService contextService) : IOperationModuleReader
{
    ...
    public async Task<OperationDto> GetAsync(Guid? departmentId, CancellationToken cancellationToken)
    {
        ...
        var rows = classrooms.Select(x => new ClassroomOperationDto(x.Id, x.Code, x.RoomType, char.IsDigit(x.Code.FirstOrDefault()) ? x.Code[0] - '0' : 1, x.Building, x.Capacity, x.DeviceOnline ? "Online" : "Offline", inStudyRoomIds.Contains(x.Id) && x.Status is not ("Offline" or "Starting") ? "In Study" : x.Status)).ToList();
        ...
        return new OperationDto(Module, $"Learning-space operations · {context.Scope}", "Live study status for classrooms and meeting rooms in the selected department.", metrics, context.Activity, context.Attention, Classrooms: rows);
    }
}
