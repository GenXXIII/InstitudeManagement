...
public sealed class TimetableCatalogService(InstituteDbContext db, InstituteCache cache) : CatalogFeatureBase<TimetableResponseDto>(db, cache), ITimetableCatalogService
{
    ...
    public override async Task<IReadOnlyList<TimetableResponseDto>> GetAsync(string? search, Guid? departmentId, CancellationToken ct)
    {
        ...
        return entries.Where(entry => Matches(search, entry.TimetableCode, entry.Course?.CourseCode, entry.Course?.Name, entry.Teacher?.TeacherCode, entry.Teacher?.FullName, entry.Classroom?.ClassroomCode, entry.Status))
            .Select(entry => new TimetableResponseDto(entry.Id, new TimetableValuesDto(
                entry.TimetableCode,
    ...
    public override async Task<TimetableResponseDto> CreateAsync(Dictionary<string, string> values, CancellationToken ct) { var entity = new ScheduleEntry(); await ApplyAsync(entity, values, ct); return await SaveCreatedAsync(entity, values, ct); }
    public override async Task<TimetableResponseDto> UpdateAsync(Guid id, Dictionary<string, string> values, CancellationToken ct) { var entity = await RequiredEntityAsync(Db.ScheduleEntries, id, ct); await ApplyAsync(entity, values, ct); Touch(entity); return await SaveUpdatedAsync(id, values, ct); }
