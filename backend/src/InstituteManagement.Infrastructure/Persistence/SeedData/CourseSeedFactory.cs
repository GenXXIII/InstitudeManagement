...
public static class CourseSeedFactory
{
    public static Course[] Create(Department[] departments, Teacher[] teachers)
    {
        ...
        return names.Select((name, index) => new Course { Code = $"{departments[index % departments.Length].Code}-{101 + index}", Name = name, DepartmentId = departments[index % departments.Length].Id, TeacherId = teachers[index].Id, Capacity = 35 + (index % 3) * 5 }).ToArray();
    }
}
