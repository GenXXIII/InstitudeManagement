...
export type StudentValues = Record<string, string> & { photoDataUrl: string; studentCode: string; publicId: string; name: string; email: string; departmentId: string; department: string; year: string; shift: string; status: string };
...
export type PublishedSemesterResult = { studentId: string; studentCode: string; fullName: string; departmentId: string; department: string; year: number; shift: string; academicYear: string; semester: string; presentCount: number; absentCount: number; permissionCount: number; grades: PublishedCourseResult[]; totalCourses: number; totalScore: number; average: number; totalGrade: string; publicationStatus: "Published"; isPublished: true; publishedAtUtc: string };
