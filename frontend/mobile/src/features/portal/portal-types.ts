...
export type StudentPayment = { id: string; paymentCode: string; financialAccountCode: string; studentId: string; academicYear: string; semester: string; totalDue: number; totalPaid: number; balance: number; amountDue: number; currency: string; dueOn: string; status: 'Pending' | 'Partial' | 'Paid' | 'Cancelled' | 'Refunded'; confirmationMethod: string; paidAtUtc: string | null; reminderSentAtUtc: string | null; reminderReadAtUtc: string | null; timetableStatus: 'Ready' | 'Waiting'; qrPayload: string };
