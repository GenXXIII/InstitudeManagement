...
export function ProfileScreen({ role }: { role: MobileRole }) {
  ...
  return <PortalPage title="My account" subtitle={`This profile belongs only to the ${role} mobile workspace.`}>
    ...
    <Card style={styles.detailCard}>
      <DetailRow icon="mail-outline" label="Login email" value={session?.email ?? ''}/>
...
const styles = StyleSheet.create({
  ...
  detailIcon: { width: 32, height: 32, borderRadius: 10, backgroundColor: palette.bluePale, alignItems: 'center', justifyContent: 'center' },
  detailCopy: { flex: 1 },
  detailLabel: { color: palette.muted, fontSize: 9, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: '700' },
  detailValue: { color: palette.ink, fontSize: 12, fontWeight: '800', marginTop: 3 },
  connectionTitle: { color: palette.ink, fontSize: 12, fontWeight: '900' },
  ...
  signOut: { minHeight: 50, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: radius.medium, backgroundColor: palette.redPale, borderWidth: 1, borderColor: '#F1C5CC' },
  signOutText: { color: palette.red, fontSize: 12, fontWeight: '900', textTransform: 'capitalize' },
  pressed: { opacity: 0.7 },
});
