...
import type { GradeItem, StudentItem } from '../portal-types';
    ...
    {courses.length ? <>
      ...
      <View style={portalStyles.stack}>{roster.map(student => {
        ...
        return <Card key={student.id}>
          <Identity photo={student.values.photoDataUrl} name={student.values.name} detail={`${student.values.studentCode} · ${existing ? `Total ${existing.values.score}/100 (${existing.values.grade})` : 'Not submitted'}`} trailing={existing ? <StatusPill value={existing.values.grade}/> : undefined}/>
          <View style={styles.attendanceEvidence}><View><Text style={styles.evidenceLabel}>Attendance · automatic</Text><Text style={styles.evidenceDetail}>{existing?.values.attendancePresent ?? '0'}/{existing?.values.attendanceSessions ?? '0'} held sessions present</Text></View><Text style={styles.evidenceScore}>{existing?.values.attendanceScore ?? '0'}/{portal.gradeWeights.attendance}</Text></View>
...
const styles = StyleSheet.create({
  courseTabs: { gap: 7, paddingRight: 8 },
  courseTab: { width: 140, padding: 11, borderRadius: radius.small, borderWidth: 1, borderBottomWidth: 2, borderColor: palette.line, backgroundColor: 'white' },
  courseTabActive: { borderColor: palette.blue, backgroundColor: '#F8FAFD' },
  courseTabCode: { color: palette.blue, fontSize: 9, fontWeight: '700' },
  courseTabName: { color: palette.ink, fontSize: 12, fontWeight: '700', marginTop: 4 },
  ...
  draftCopy: { flex: 1 },
  draftTitle: { color: palette.ink, fontSize: 12, fontWeight: '700' },
  draftDetail: { color: palette.muted, fontSize: 10, lineHeight: 15, marginTop: 3 },
  ...
  submitAllText: { color: 'white', fontSize: 11, fontWeight: '700' },
  ...
  evidenceLabel: { color: palette.ink, fontSize: 10, fontWeight: '700' },
  evidenceDetail: { color: palette.muted, fontSize: 9, marginTop: 3 },
  evidenceScore: { color: palette.blueDark, fontSize: 15, fontWeight: '800' },
  componentInputs: { gap: 9, marginTop: 11 },
  componentInput: { gap: 5 },
  componentLabel: { color: palette.ink, fontSize: 10, fontWeight: '700' },
  ...
  maximum: { width: 36, color: palette.muted, fontSize: 10, fontWeight: '700' },
  ...
  saveText: { color: 'white', fontWeight: '700', fontSize: 10 },
  ...
  resultCopy: { flex: 1 },
  resultCourse: { color: palette.ink, fontWeight: '700', fontSize: 13 },
  ...
  resultComponentLabel: { color: palette.ink, fontSize: 11, fontWeight: '600' },
  resultComponentScore: { color: palette.ink, fontSize: 11, fontWeight: '700' },
  resultComponentDetail: { width: '100%', color: palette.muted, fontSize: 9 },
});
