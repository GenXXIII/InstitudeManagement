...
export default function DashboardPage() {
  ...
  const load = useCallback(() => { api.dashboard().then(setData).catch(() => setError(true)); }, []);
  useEffect(load, [load]);
  if (error) return <ErrorPage retry={load}/>; if (!data) return <LoadingPage/>;
  const now = new Date();
  return <>
    <PageHeading eyebrow="Institute dashboard" title={`Good morning, Alex`} description={now.toLocaleDateString("en-US", { weekday: "long", day: "numeric", month: "long", year: "numeric" })} actions={<><span className="live-pill"><i/> System online</span><button className="button secondary" onClick={load}>Refresh</button></>}/>
    ...
    <section className="dashboard-grid">
      ...
      <article className="panel activity-panel"><div className="panel-title"><div><span className="panel-kicker">Just now</span><h3>Live activity</h3></div><Link href="/records/audit-logs">History</Link></div><ActivityList items={data.activity}/></article>
      ...
      <article className="panel grade-panel"><div className="panel-title"><div><span className="panel-kicker">Academic status</span><h3>Grade distribution</h3></div><strong className="average">82.4 <small>avg.</small></strong></div><div className="bars">{data.gradeDistribution.map(item => <div key={item.label}><span>{item.label}</span><i style={{height: `${Math.max(item.value * 2, 24)}px`}}/><b>{item.value}%</b></div>)}</div></article>
    </section>
  </>;
}

function linePath(points: { value: number }[]) { return points.map((p, i) => `${i ? "L" : "M"} ${i * 120} ${160 - (p.value - 75) * 6}`).join(" "); }
function areaPath(points: { value: number }[]) { return `${linePath(points)} L ${(points.length - 1) * 120} 170 L 0 170 Z`; }
