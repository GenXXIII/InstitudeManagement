...
const descriptions: Record<string, string> = { "attendance-rules": "Choose how attendance is captured and how exceptions should be handled.", "grade-rules": "Set the score boundaries used to calculate letter grades.", notifications: "Control which operational events create institute notifications." };
...
export default function SettingsPage() {
  ...
  const load = useCallback(() => { api.settings(section).then(result => { setData(result); setValues(result.values); }).catch(() => setError(true)); }, [section]); useEffect(load, [load]);
  if (error) return <ErrorPage retry={load}/>; if (!data) return <LoadingPage/>;
  async function save() { setSaving(true); setSaveError(""); try { const result = await api.saveSettings(section, values); setValues(result.values); setSaved(true); setTimeout(() => setSaved(false), 2500); } catch (reason) { setSaveError(reason instanceof Error ? reason.message : "Could not save these settings."); } finally { setSaving(false); } }
...
function SettingsPreview({ section, values }: { section: string; values: Record<string, string> }) {
  ...
  if (section === "institute") return <><span className="preview-kicker">Public institute identity</span><div className="institute-preview-mark">{values.shortName || "NSI"}</div><h3>{values.name}</h3><p>{values.address}</p><div className="identity-contact"><span>{values.email}</span><span>{values.phone}</span></div></>;
  if (section === "academic-year" || section === "semester") return <><span className="preview-kicker">Active academic window</span><div className="term-preview"><Icon name="calendar" size={22}/><strong>{values.currentYear ?? values.currentTerm}</strong><span>Current {section === "semester" ? "term" : "academic year"}</span></div><div className="term-track"><i/><i/><i/></div><div className="term-dates"><span><small>Starts</small>{values.startsOn}</span><span><small>Ends</small>{values.endsOn}</span></div></>;
  ...
  if (section === "grade-rules") return <><span className="preview-kicker">Letter grade boundaries</span><div className="grade-rule-preview">{["a", "b", "c", "d"].map(letter => <div key={letter} className={`grade-${letter}`}><strong>{letter.toUpperCase()}</strong><span>{values[`${letter}Minimum`]}+</span></div>)}</div><p>Scores below the D minimum become F.</p></>;
