...
export function groupRecords(rows: RecordItem[]): RecordGroup[] {
  ...
  return [...grouped.entries()].map(([key, entries]) => { entries.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()); const current = entries.find(entry => entry.resourceId && entry.id === entry.resourceId); const terminal = entries.find(entry => isInactive(entry.action)); return { key, type: current?.type ?? entries[0].type, subject: current?.subject ?? entries[0].subject, status: current?.action ?? terminal?.action ?? "Historical", businessCode: entries.find(entry => entry.businessCode)?.businessCode ?? "", entries, values: parseDetails((current ?? entries[0]).details) }; }).sort((a, b) => new Date(b.entries[0].date).getTime() - new Date(a.entries[0].date).getTime());
}
