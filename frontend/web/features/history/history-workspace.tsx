...
function RecordRegister() {
  ...
  const groups = useMemo(() => groupRecords(rows), [rows]);
  ...
  return <>
    ...
    {visible.length ? <section className="record-register panel"><div className="record-register-head"><span>Record</span><span>Latest details</span><span>Status</span><span>Last updated</span><span>History</span></div><div className="record-register-list">{visible.map(group => <RecordRow group={group} key={group.key}/>)}</div></section> : <section className="panel empty-state"><div className="empty-icon"><Icon name="archive" size={28}/></div><strong>No records found</strong><span>Try another search or status filter.</span></section>}
  </>;
}
