  ...
  return <div className="modal-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}><form className="modal management-modal" onSubmit={save} noValidate>
    ...
    <div className="management-form-grid">
      ...
      {fields.map(field => field.type === "select"
        ? <div className="editor-field relationship-editor-field" key={field.key}><span>{field.label}</span><SearchableSelect value={displayedFieldValue(field, values, creating)} options={field.options ?? []} placeholder={selectPlaceholder(field)} ariaLabel={field.label} required={field.required} onChange={value => changeField(field, value)}/></div>
        : <label className="editor-field" key={field.key}><span>{field.label}</span><input readOnly={field.readOnly} type={field.type === "time" ? "time" : field.type === "number" ? "number" : "text"} min={field.type === "number" ? "1" : undefined} value={values[field.key] ?? ""} onChange={event => changeField(field, event.target.value)}/></label>)}
    </div>
