...
export function OperationalRecordWorkspace({ module: rawModule, history = false }: { module: string; history?: boolean }) {
  ...
  return <div className="viewport-data-page record-viewport-page">
    <PageHeading eyebrow={history ? "Read-only semester record history" : "Active-semester records"} title={history ? `${config.title} history` : config.title} description={`${history ? "The current semester mirrors Record automatically; closed semesters remain permanent and read-only. " : "Edit the current semester here. When the semester advances, this view resets while its old data remains in Record History. "}${config.description}${year ? ` Showing Year ${year}.` : ""}`}/>
