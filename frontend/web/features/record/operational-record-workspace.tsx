...
export function OperationalRecordWorkspace({ module: rawModule, history = false }: { module: string; history?: boolean }) {
  ...
  return <div className="viewport-data-page record-viewport-page">
    ...
    <section className="record-toolbar panel"><label className="record-search management-search module-search-field"><Icon name="search" size={16}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder={`Search ${config.title.toLowerCase()}…`} aria-label={`Search ${config.title}`}/></label><span className="record-count">Showing {visibleRows.length} records</span></section>
