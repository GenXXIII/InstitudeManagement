...
export function OperationalRecordWorkspace({ module: rawModule, history = false }: { module: string; history?: boolean }) {
  ...
  return <div className="viewport-data-page record-viewport-page">
    ...
    {history && <section className="record-semester-switcher panel"><div><span>{isStudentModule ? "Graduate archive" : "Semester archive"}</span><strong>{selectedPeriod === "all" ? isStudentModule ? "Complete Year 1–4 history" : "All completed semesters" : periods.find(period => period.key === selectedPeriod)?.label ?? "Selected semester"}</strong></div><label><span>Filter archived semester</span><select value={selectedPeriod} onChange={event => changePeriod(event.target.value)}><option value="all">{isStudentModule ? "Full four-year archive" : "All completed semesters"}</option>{periods.map(period => <option value={period.key} key={period.key}>{period.label}</option>)}</select></label></section>}
    ...
    {visibleRows.length ? <section className="record-paginated-region"><div className="semester-history-scroll">{periodGroups.map(group => <SemesterHistoryGroup module={currentModule} group={group} history={history} load={load} detailHref={detailHref} key={group.key}/>)}</div></section> : <section className="panel empty-state"><div className="empty-icon"><Icon name="archive" size={28}/></div><strong>{history ? isStudentModule ? "No completed four-year archive yet" : "No completed semester history yet" : "No current semester records found"}</strong><span>{history ? isStudentModule ? "A student moves here only after completing Year 4 Semester 2." : "This module receives a read-only entry after each semester closes." : isStudentModule ? "Student records accumulate here from enrollment through Year 4 Semester 2." : "Current-semester records appear automatically from Enrollment and timetable activity."}</span></section>}
  </div>;
