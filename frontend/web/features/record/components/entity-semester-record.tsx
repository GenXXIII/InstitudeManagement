...
function EntitySessionCard({ session, module }: { session: Record<string, string>; module: RecordModule }) {
  ...
  return <article className={`entity-session-card ${classHeld ? "" : "class-not-held"}`}><div className="entity-session-main"><div><time>{session.Date}</time><strong>{session.Time}</strong><span>{session["Session status"]}</span></div><div><strong>{title || "Recorded class"}</strong><span>{context}</span><small>{session.Reason}</small></div>{classHeld ? <AttendanceCards counts={counts}/> : <div className="entity-session-not-held"><strong>Not held</strong><span>{session["Teacher attendance"]}</span></div>}</div>{students.length > 0 && <details><summary>View {students.length} student attendance details</summary><div className="entity-student-snapshot">{students.map((student, index) => <span className={`attendance-${toneKey(student.status)}`} key={`${student.name}-${index}`}><strong>{student.name}</strong><b>{attendanceLabel(student.status)}</b></span>)}</div></details>}</article>;
}
