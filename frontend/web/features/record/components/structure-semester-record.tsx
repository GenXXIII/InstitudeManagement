...
export function StructureSemesterRecordHeader({ module }: { module: StructureModule }) {
  return module === "Department"
    ? <div className="structure-semester-record-head"><span>Department code</span><span>Type</span><span>Department</span><span>Head</span><span>Year</span><span>Students / teachers</span><span>Courses / rooms</span><span>Completed classes</span></div>
...
export function StructureSemesterRecord({ row, stage = "record", detailHref, detailPage = false }: { row: OperationalRecord; stage?: WorkflowCodeStage; detailHref?: string; detailPage?: boolean }) {
  ...
  return <article className="structure-semester-record-row record-row-clickable" role="link" tabIndex={0} onClick={open} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); open(); } }}>
    ...
    {recordModule === "Department" ? <SingleMetric value={completed.length} label="completed classes"/> : <PairMetric first={summary["Student count"]} firstLabel="students" second={completed.length.toString()} secondLabel="classes"/>}
  </article>;
}
...
function StructureRecordDetail({ row, stage }: { row: OperationalRecord; stage: WorkflowCodeStage }) {
  ...
  return <article className="structure-semester-detail">
    ...
    <section className="structure-session-register"><header><div><strong>Completed timetable history</strong><span>Visual class details recorded under this semester</span></div><b>{completed.length} classes</b></header><div>{completed.length ? completed.map((activity, index) => <article key={`${activity.Date}-${activity.Time}-${index}`}><div><time>{activity.Date}</time><strong>{activity.Time}</strong></div><div><strong>{activity.Course || row.subject}</strong><span>{activity.Teacher} · Room {activity.Classroom} · {activity.Year}</span></div><div><strong>{activity["Student count"] || studentCount(activity).toString()}</strong><span>students</span></div><p>{activity.Attendance || "Attendance summary unavailable"}</p></article>) : <p className="student-record-detail-empty">No completed classes in this semester.</p>}</div></section>
  </article>;
}
...
function studentCount(activity: Record<string, string>) { return (activity.Students ?? "").split("; ").filter(Boolean).length; }
