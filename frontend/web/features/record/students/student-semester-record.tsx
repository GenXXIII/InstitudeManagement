...
function StudentSemesterDetail({ row, stage, insights, gradeSlots, editable, onUpdated }: { row: OperationalRecord; stage: WorkflowCodeStage; insights: OperationalRecordInsights; gradeSlots: Array<OperationalRecordGrade | null>; editable: boolean; onUpdated?: () => void }) {
  ...
  return <article className="student-semester-detail student-active-semester-detail">
    ...
    <section className="student-record-detail-section student-course-grades-section"><header><div><strong>Course grades</strong><span>{insights.expectedCourses} course-grade records preserved for this semester</span></div><b>{insights.grades.length}/{insights.expectedCourses} assigned</b></header><div className="student-grade-detail-grid">{gradeSlots.map((grade, index) => <GradeDetailCard grade={grade} index={index} studentId={row.resourceId} stage={stage} editable={editable} onUpdated={onUpdated} key={grade?.courseCode ?? index}/>)}</div><footer className={`student-final-result result-${toneKey(insights.result)}`}><div><span>Total score</span><strong>{insights.totalScore.toFixed(1)} ÷ {insights.expectedCourses} = {insights.average.toFixed(2)}</strong></div><b>{insights.result}</b></footer></section>
...
function ResultCard({ insights }: { insights: OperationalRecordInsights }) { return <div className={`student-result-card result-${toneKey(insights.result)}`}><strong>{insights.result}</strong><span>{insights.totalScore.toFixed(1)} ÷ {insights.expectedCourses}</span><b>{insights.average.toFixed(2)}</b></div>; }
