...
export function ResultWorkspace({ mode }: { mode: ResultMode }) {
  ...
  return <>
    ...
    {visible.length ? <section className="panel semester-result-table"><div className="semester-result-head"><span>Student</span><span>Year / semester</span><span>Attendance</span><span>Course grades</span><span>Total</span><span>Average</span><span>Total grade</span></div><div>{visible.map(row => <ResultRow row={row} key={`${row.studentId}-${row.academicYear}-${row.semester}`}/>)}</div></section> : <section className="panel empty-state"><div className="empty-icon"><Icon name="grade" size={28}/></div><strong>No semester results found</strong><span>Change the year, semester, or search filters.</span></section>}
  </>;
}
