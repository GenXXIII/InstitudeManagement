...
const copy: Record<ResultMode, { eyebrow: string; title: string; description: string }> = {
  history: { eyebrow: "Permanent graduate history", title: "Result Semester", description: "After Year 4 Semester 2 graduation, every semester result from Year 1 through Year 4 remains here as read-only history." },
};
...
export function ResultWorkspace({ mode }: { mode: ResultMode }) {
  ...
  return <div className="viewport-data-page result-viewport-page">
    ...
    <section className="record-toolbar panel result-toolbar"><label className="record-search management-search module-search-field"><Icon name="search" size={16}/><input value={query} onChange={event => setQuery(event.target.value)} placeholder="Search student, course, or department…" aria-label="Search results"/></label><select value={academicYear} onChange={event => setAcademicYear(event.target.value)}><option value="all">All academic years</option>{academicYears.map(value => <option value={value} key={value}>{value}</option>)}</select><select value={semester} onChange={event => setSemester(event.target.value)}><option value="all">All semesters</option>{semesters.map(value => <option value={value} key={value}>{value}</option>)}</select><select value={outcome} onChange={event => setOutcome(event.target.value)} aria-label="Result outcome"><option value="final">Completed results</option><option value="retake-exam">Retake exam</option><option value="fail">Failed</option><option value="all">All outcomes</option></select><span className="record-count">{visible.length} results</span></section>
    {visible.length ? <section className="result-paginated-region"><DataTable className="panel semester-result-table" headerClassName="semester-result-head" rowSelector=".semester-result-row" columns={["Student", "Year / semester", "Attendance", "Course grades", "Total", "Average", "Total grade"]}><div>{pagination.pageItems.map(row => <ResultRow row={row} key={`${row.studentId}-${row.academicYear}-${row.semester}`}/>)}</div></DataTable><DataPagination page={pagination.page} pageCount={pagination.pageCount} total={visible.length} onPage={pagination.setPage}/></section> : <section className="panel empty-state"><div className="empty-icon"><Icon name="grade" size={28}/></div><strong>No semester results found</strong><span>Change the year, semester, or search filters.</span></section>}
  </div>;
}
