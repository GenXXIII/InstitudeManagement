...
export function TimetableBoard({ items, references, onEdit, onDeactivate }: { items: TimetableItem[]; references: References; onEdit: (item: TimetableItem) => void; onDeactivate: (item: TimetableItem) => void }) {
  ...
  return <section className="management-timetable-data">
    ...
    <div className="panel timetable-data-table">
      <div className="timetable-data-head"><span>TimetableCode</span><span>Day & time</span><span>Course</span><span>Department / cohort</span><span>Teacher</span><span>Learning room</span><span>Students</span><span>Create At</span><span>Status</span><span>Actions</span></div>
      <div className="timetable-data-body">{visible.map(item => {
        ...
        return <article className="timetable-data-row" key={item.id}>
          <strong className="management-code-value">{item.values.timetableCode}</strong>
          ...
          <div className="timetable-detail-data"><strong>{item.values.createAt}</strong></div>
