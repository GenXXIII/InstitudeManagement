...
export function ModuleLayout({ module, items, references, onEdit, onDeactivate }: { module: Exclude<ManagementModule, "overview">; items: ManagementItem[]; references: References; onEdit: (item: ManagementItem) => void; onDeactivate: (item: ManagementItem) => void }) {
  if (module === "students") return <StudentRoster items={items as StudentItem[]} references={references} onEdit={onEdit} onDeactivate={onDeactivate}/>;
  ...
  return <Gradebook items={items as GradeItem[]} courses={references.courses} onEdit={onEdit} onDeactivate={onDeactivate}/>;
}
