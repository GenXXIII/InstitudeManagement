...
export function EditorField({ field, value, options, onChange }: { field: Field; value: string; options: { id: string; label: string }[]; onChange: (value: string) => void }) {
  ...
  return <label className="editor-field"><span>{field.label}</span>{field.type === "select"
    ? field.source
      ? <SearchableSelect required={field.required} value={value} options={[{ id: "", label: `Select ${field.label.toLowerCase()}` }, ...options]} placeholder={`Type to find ${field.label.toLowerCase()}…`} ariaLabel={field.label} onChange={onChange}/>
      : <select required={field.required} value={value} onChange={event => onChange(event.target.value)}><option value="">Select {field.label.toLowerCase()}</option>{options.map(option => <option key={option.id} value={option.id}>{option.label}</option>)}</select>
    : <input required={field.required} type={field.type ?? "text"} step={field.key === "score" ? ".1" : undefined} min={field.key === "score" ? "0" : undefined} max={field.key === "score" ? "100" : undefined} value={value} onChange={event => onChange(event.target.value)}/>}</label>;
}
