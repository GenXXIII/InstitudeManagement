...
export function DashboardOperationGrid({ data, departmentId, year }: { data: Operation; departmentId: string; year: number }) {
  ...
  const areas = [
    ...
    { name: "Teachers", route: "/operation/teachers", rows: (data.teachers ?? []).slice(0, 6).map(row => ({ title: row.teacher, detail: `${row.teacherCode} · ${row.department}`, state: row.status })) },
    { name: "Classrooms", route: "/operation/classrooms", rows: (data.classrooms ?? []).filter(row => row.status === "In Study").slice(0, 6).map(row => ({ title: `Room ${row.room}`, detail: `${row.roomType} · ${row.capacity} seats`, state: row.status })) },
  ...
  return <div className="unified-operation-dashboard">
    ...
    <footer className="unified-operation-note"><Icon name="pulse" size={16}/><div><strong>One overview, four live operational areas</strong><span>This summary refreshes automatically. Open a full view only when you need deeper management detail.</span></div></footer>
  </div>;
}
