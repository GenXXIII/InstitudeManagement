...
export function WeeklyTimetable({ rows, periods, rooms }: { rows: WeeklyTimetableSlot[]; periods: TimetablePeriod[]; rooms: TimetableRoom[] }) {
  ...
  const visibleRows = rows.filter(row => row.day === selectedDay && (!selectedYear || row.yearLevel === selectedYear));
