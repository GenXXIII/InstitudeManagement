...
export function ClassroomBuilding({ rows }: { rows: ClassroomOperation[] }) {
  return <div className="classroom-building">
    <div className="building-roof"><Icon name="building" size={18}/><div><strong>Institute learning spaces</strong><span>Timetable controls Available and Running; maintenance states stay fixed</span></div></div>
    {[5, 4, 3, 2, 1].map(floor => {
      const floorRooms = rows.filter(row => row.floor === floor);
      return <section className={`building-floor floor-${floor}`} key={floor}>
        <header><b>{floor}</b><span>Floor {floor}</span></header>
        <div className="floor-rooms">{floorRooms.length ? floorRooms.map(room => {
