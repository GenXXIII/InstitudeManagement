...
export const policyGroups = {
  ...
  "grade-rules": [
    ...
    {
      title: "Course score composition",
      description: "The four component weights must total exactly 100%. Attendance is calculated from held class sessions.",
      fields: [
        field("attendanceWeight", "Attendance weight", "Maximum points earned from present or late attendance sessions.", "number", { required: true, min: 0, max: 100, step: 0.01, unit: "%" }),
        field("assignmentWeight", "Assignment weight", "Maximum points teachers can enter for assignment work.", "number", { required: true, min: 0, max: 100, step: 0.01, unit: "%" }),
        field("midtermWeight", "Midterm weight", "Maximum points teachers can enter for the midterm exam.", "number", { required: true, min: 0, max: 100, step: 0.01, unit: "%" }),
        field("finalExamWeight", "Final exam weight", "Maximum points teachers can enter for the final exam.", "number", { required: true, min: 0, max: 100, step: 0.01, unit: "%" }),
        field("componentTotal", "Component total", "Must equal 100% before settings can be saved.", "derived", { derive: values => `${["attendanceWeight", "assignmentWeight", "midtermWeight", "finalExamWeight"].reduce((total, key) => total + Number(values[key] || 0), 0)}%` }),
      ],
    },
    {
      title: "Grade scale",
