...
export function SettingsSection({ section }: { section: SettingSection }) {
  ...
  const visibleGroups = useMemo(() => {
    if (advanced) return definition.groups;
  ...
  const hiddenFieldCount = Math.max(0, editableFields.length - visibleEditableCount);
  const changedCount = useMemo(() => values ? editableFields.filter(item => values[item.key] !== baseline[item.key]).length : 0, [baseline, editableFields, values]);
  ...
  async function save() {
    const pendingValues = normalizeSection(section, values!);
  ...
  function restoreRecommended() {
    ...
    setValues(normalizeSection(section, { ...unknown, ...defaultSettings[section] }));
    setValidationErrors([]);
    setSaveError("");
    setSaved(false);
  }
  ...
  return <div className="viewport-data-page administration-page administration-section-page">
    ...
    <section className="administration-section-scroll">
        ...
        <article className="administration-editor">
          <header><div><span>{advanced ? "Advanced configuration" : "Simple configuration"}</span><h2>Review and apply</h2><p>{advanced ? `All ${editableFields.length} available settings are shown.` : `${visibleEditableCount} essential settings are shown. ${hiddenFieldCount} advanced settings remain safely unchanged.`}</p></div><button type="button" className="button secondary" onClick={restoreRecommended}>Recommended defaults</button></header>
          ...
          <footer className="administration-save-bar">
            <div><span className={saved ? "administration-save-message show" : "administration-save-message"}>Settings saved successfully.</span>{!saved && <span>{dirty ? `${changedCount} unsaved ${changedCount === 1 ? "change" : "changes"}` : configured ? `Saved · ${formatUpdatedAt(updatedAtUtc)}` : "Review and apply the sample defaults to store this section."}</span>}</div>
            <button className="button secondary" type="button" onClick={() => setValues({ ...baseline })} disabled={!dirty || saving}>Undo</button>
            <button className="button primary" type="button" onClick={() => void save()} disabled={saving || (configured && !dirty)}>{saving ? "Applying…" : "Apply settings"}</button>
          </footer>
        </article>
