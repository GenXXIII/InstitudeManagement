...
export default function SettingsPage() {
  ...
  const orderedKeys = section === "semester"
  ...
  const settingEntries = orderedKeys.length ? orderedKeys.filter(key => key in values).map(key => [key, values[key]] as const) : Object.entries(values);
  return <>
    ...
    <section className="settings-layout">
      <article className="panel settings-panel"><div className="settings-intro"><div><h3>{titles[section] ?? "Settings"}</h3><p>Changes are saved to the institute database and recorded in the audit history.</p></div><span className="settings-badge">Current configuration</span></div>
        {hasValues ? <div className={`settings-form settings-form-${section}`}>{settingEntries.map(([key, value]) => <SettingField key={key} name={key} value={value} onChange={next => setValues({ ...values, [key]: next })}/>) }{section === "grade-rules" && <label className="setting-field grade-f-rule"><span>F</span><input value={`Below ${values.eMinimum || 0} = F`} readOnly aria-label="F grade rule"/></label>}</div> : <div className="empty-state"><strong>No custom options yet</strong><span>This section uses the institute-wide defaults.</span></div>}
