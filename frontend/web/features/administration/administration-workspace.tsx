...
export default function SettingsPage() {
  ...
  async function save() { setSaving(true); setSaveError(""); try { const result = await administrationApi.save(section, values); setValues(result.values); setSaved(true); setTimeout(() => setSaved(false), 2500); } catch (reason) { setSaveError(reason instanceof Error ? reason.message : "Could not save these settings."); } finally { setSaving(false); } }
