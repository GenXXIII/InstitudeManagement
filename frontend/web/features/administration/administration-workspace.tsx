...
import { settingsTemplates, type SettingSection, useInstituteSettings } from "./institute-settings-context";
...
export default function SettingsPage() {
  ...
  async function save() { const problems = validateSettings(section, values); setValidationErrors(problems); setSaveError(""); if (problems.length) return; setSaving(true); try { const result = await administrationApi.save(section, values); setValues(result.values); setConfigured(true); await refresh(); setSaved(true); setTimeout(() => setSaved(false), 2500); } catch (reason) { setSaveError(reason instanceof Error ? reason.message : "Could not save these settings."); } finally { setSaving(false); } }
  const hasValues = Object.keys(values).length > 0;
  const orderedKeys = section === "academic-year"
    ? ["currentYear", "startsOn", "endsOn"]
    : section === "semester"
    ? ["currentTerm", "startsOn", "endsOn", "semester1StartsOn", "semester1EndsOn", "semester2StartsOn", "semester2EndsOn"]
    : section === "grade-rules" ? ["aMinimum", "bMinimum", "cMinimum", "dMinimum", "eMinimum"] : [];
  const standardKeys = orderedKeys.length ? orderedKeys : Object.keys(template);
  const customKeys = Object.keys(values).filter(key => !Object.hasOwn(template, key));
  const settingEntries = [...standardKeys.filter(key => key in values), ...customKeys].map(key => [key, values[key]] as const);
  return <div className="viewport-data-page administration-viewport-page">
    ...
    <section className="settings-layout">
      <article className="panel settings-panel"><div className="settings-intro"><div><h3>{titles[section] ?? "Settings"}</h3><p>Changes are saved to the institute database and recorded in the audit history.</p></div><div className="settings-intro-actions"><span className={`settings-badge ${configured ? "" : "pending"}`}>{configured ? "Current configuration" : "Not configured"}</span></div></div>
        {hasValues ? <div className={`settings-form settings-form-${section}`}>{settingEntries.map(([key, value]) => <div className={`setting-field-wrap ${value === "true" || value === "false" ? "wide" : ""}`} key={key}><SettingField name={key} value={value} onChange={next => { setValues({ ...values, [key]: next }); setValidationErrors([]); setSaveError(""); }}/></div>) }{section === "grade-rules" && <label className="setting-field grade-f-rule"><span>F</span><input value={`Below ${values.eMinimum || 0} = F`} readOnly aria-label="F grade rule"/></label>}</div> : <div className="empty-state"><strong>No configuration fields</strong></div>}
        {(validationErrors.length > 0 || saveError) && <div className="form-error validation-summary" role="alert"><strong>Fix these problems:</strong><ul>{[...validationErrors, ...(saveError ? [saveError] : [])].map(problem => <li key={problem}>{problem}</li>)}</ul></div>}<div className="settings-actions"><span className={saved ? "save-message show" : "save-message"}>✓ Changes saved</span><button className="button secondary" onClick={load}>Cancel</button><button className="button primary" onClick={save} disabled={!hasValues || saving}>{saving ? "Validating…" : configured ? "Save changes" : "Create configuration"}</button></div>
      </article>
...
function SettingsPreview({ section, values }: { section: string; values: Record<string, string> }) {
  ...
  if (section === "courses") return <><span className="preview-kicker">New course defaults</span><div className="course-rule-preview"><div>NEW</div><h3>Course template</h3><p>Teacher relationship {enabled("requireAssignedTeacher") ? "required" : "optional"}</p><span><b>{values.defaultCapacity}</b> seats</span></div></>;
  if (section === "classrooms") return <><span className="preview-kicker">Shared classroom defaults</span><div className="classroom-rule-preview"><Icon name="room" size={24}/><strong>Institute room template</strong><b>{values.defaultCapacity}</b><span>default seats</span></div><RulePill active={enabled("attendanceDeviceRequired")} label="Device required"/></>;
...
function SettingField({ name, value, onChange }: { name: string; value: string; onChange: (value: string) => void }) {
  const label = /^[a-e]Minimum$/.test(name) ? name[0].toUpperCase() : name.replace(/([A-Z])/g, " $1").replace(/^./, value => value.toUpperCase());
  ...
  if (name === "language") return <label className="setting-field"><span>{label}</span><select value={value} onChange={e => onChange(e.target.value)}><option>English</option><option>Khmer</option></select></label>;
  if (name === "dateFormat") return <label className="setting-field"><span>{label}</span><select value={value} onChange={e => onChange(e.target.value)}><option>DD MMM YYYY</option><option>DD/MM/YYYY</option><option>MM/DD/YYYY</option><option>YYYY-MM-DD</option></select></label>;
  if (name === "timeZone") { const options = timeZones.includes(value) ? timeZones : [value, ...timeZones]; return <label className="setting-field"><span>{label}</span><select value={value} onChange={e => onChange(e.target.value)}>{options.map(zone => <option key={zone} value={zone}>{zone.replaceAll("_", " ")}</option>)}</select></label>; }
  if (name === "currentYear") { const options = academicYears.includes(value) ? academicYears : [value, ...academicYears]; return <label className="setting-field setting-field-current-year"><span>Current year</span><select value={value} onChange={e => onChange(e.target.value)}>{options.map(year => <option key={year} value={year}>{year}</option>)}</select></label>; }
  if (name === "currentTerm") return <label className="setting-field setting-field-semester"><span>Semester</span><select value={value} onChange={e => onChange(e.target.value)}><option>Semester 1</option><option>Semester 2</option></select></label>;
