...
function AnnounceOverview({ notifications, alerts, history }: { notifications: NotificationItem[]; alerts: AnnouncementItem[]; history: NotificationHistoryItem[] }) {
  const recent = [
    ...
    ...history.map(item => ({ code: item.notificationHistoryCode, kind: "History", title: item.title, detail: `${item.sourceCode} · ${item.action}`, date: item.createAt, href: `/announce/history/${item.id}` })),
...
function HistoryRegister({ rows, onOpen }: { rows: NotificationHistoryItem[]; onOpen: (id: string) => void }) {
  return <section className="panel notification-inbox notification-history-register"><div className="notification-inbox-head"><span/><span>HistoryCode</span><span>SourceCode</span><span>Type</span><span>Title and preview</span><span>Recorded</span><span>Action</span></div>{rows.map(item => <article className="notification-inbox-row" role="link" tabIndex={0} onClick={() => onOpen(item.id)} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") onOpen(item.id); }} key={item.id}><i/><strong className="management-code-value">{item.notificationHistoryCode}</strong><strong className="management-code-value">{item.sourceCode}</strong><span className={`table-status alert-${item.type.toLowerCase()}`}>{item.type}</span><div className="notification-inbox-copy"><strong>{item.title}</strong><span>{item.message}</span></div><time>{new Date(item.createAt).toLocaleDateString()}</time><span className="table-status">{item.action}</span></article>)}{!rows.length && <div className="empty-state"><strong>No read history</strong><span>Notifications move here after they are opened.</span></div>}</section>;
}
