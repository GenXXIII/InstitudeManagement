...
- **Management** is the only place for adding, editing, deactivating, or removing current data. Saving a Student or Teacher automatically creates a permanent Public ID used for mobile login. Every module can be scoped by department.
...
Scan the LAN QR code in Expo Go. Students and Teachers sign in with their automatically generated Public ID and the initial password `1234`. See [`frontend/mobile/README.md`](frontend/mobile/README.md) for the API-address fallback.
